...
$col_widths = array(8, 30, 30, 40, 15, 20, 25, 25, 25, 35, 25);
...
foreach ($data_kependudukan as $penduduk) {
    ...
    if (strlen($pekerjaan) > 15) {
        $pekerjaan = substr($pekerjaan, 0, 15) . '...';
    }

    // Format nama
    $nama = substr($penduduk['NAMA_LGKP'], 0, 25);

    $pdf->Cell($col_widths[0], 6, $no++, 1, 0, 'C');  // No
    $pdf->Cell($col_widths[1], 6, ($penduduk['NO_KK']), 1);  // No. KK
    $pdf->Cell($col_widths[2], 6, ($penduduk['NIK']), 1);  // NIK
    $pdf->Cell($col_widths[3], 6, $nama, 1);  // Nama
    $pdf->Cell($col_widths[4], 6, $penduduk['JK'], 1, 0, 'C');  // JK
    $pdf->Cell($col_widths[5], 6, substr($penduduk['HBKEL'], 0, 8), 1, 0, 'C');  // Hub. Kel
    $pdf->Cell($col_widths[6], 6, substr($penduduk['TMPT_LHR'], 0, 12), 1);  // Tempat Lahir
    $pdf->Cell($col_widths[7], 6, $tgl_lahir_formatted, 1, 0, 'C');  // Tanggal Lahir
    $pdf->Cell($col_widths[8], 6, substr($penduduk['PENDIDIKAN'], 0, 10), 1);  // Pendidikan
    $pdf->Cell($col_widths[9], 6, $pekerjaan, 1);  // Pekerjaan
    $pdf->Cell($col_widths[10], 6, substr($penduduk['dusun'], 0, 10), 1, 0, 'C');  // Dusun
